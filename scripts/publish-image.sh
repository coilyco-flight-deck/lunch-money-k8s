...
image_name="coilyco-flight-deck/lunch-money-mcp"
...
case "${sha}" in
  *[!0-9a-f]*|"")
    echo "lunch-money-mcp source sha is not a lowercase hexadecimal commit id." >&2
    exit 1
    ;;
esac
if [ "${#sha}" -ne 40 ]; then
  echo "lunch-money-mcp source sha must be a full 40-character commit id." >&2
  exit 1
fi
